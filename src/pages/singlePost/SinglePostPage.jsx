...
function SinglePostPage(){
    ...
    const {isPending,error,data}=useQuery({
        ...
        queryFn:async()=>{
            const res=await axios.get(import.meta.env.VITE_SERVER_ENDPOINT+`/api/posts/${id}`)
            return res.data        
        }
    })
    ...
    async function handleSavePost(){
        ...
        try{
        const res=await axios.post(import.meta.env.VITE_SERVER_ENDPOINT+"/api/users/post/save",{postId:id})
    ...
    async function handleSend(){
        if(!userAuth)return navigate("/login")
        try{
           const response=await axios.post(import.meta.env.VITE_SERVER_ENDPOINT+"/api/chats",{receiverId:data?.user["_id"]})
